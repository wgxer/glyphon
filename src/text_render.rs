                ...
                for glyph in run.glyphs.iter() {
                    ...
                    let (gpu_cache, atlas_id, inner) = if should_rasterize {
                        ...
                        for row in 0..height {
                            ...
                            let x_offset =
                                (y_offset + row) * inner.size as usize + atlas_min.x as usize;
            ...
            if let Some(ub) = bounds {
                ...
                queue.write_texture(
                    ...
                    &inner.texture_pending
                        [ub.y_min * inner.size as usize + ub.x_min * num_atlas_channels..],
                    ImageDataLayout {
                        offset: 0,
                        bytes_per_row: NonZeroU32::new(inner.size * num_atlas_channels as u32),
                        rows_per_image: NonZeroU32::new(inner.size),
                    },
                    Extent3d {
